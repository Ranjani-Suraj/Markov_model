...
public class ConnGraph {
     ...
     * number is 0.
     */
    private int maxLogVertexCountSinceRebuild;  //log(n) where n is the max vertices since last rebuilt. 
    //rebiuld is only when there are significantly less vertices in the graph. nothign to do w the actual functioning. just optimizing space
    ...
    private VertexInfo ensureInfo(ConnVertex vertex) {
        ...
        if (info != null) {  //if vretex in vertexInfo
            return info;
        }

        if (vertexInfo.size() == MAX_VERTEX_COUNT) { //if there are more vertices than int max
            throw new RuntimeException(
        ...
        EulerTourNode node = new EulerTourNode(eulerTourVertex, augmentation); //create new ett node
        eulerTourVertex.arbitraryVisit = node; 
        node.left = EulerTourNode.LEAF;  //will be new ETN(null, null)
        node.right = EulerTourNode.LEAF;
        node.augment();
        //so basically an empty ETT node

        info = new VertexInfo(eulerTourVertex);
        vertexInfo.put(vertex, info);
        if (vertexInfo.size() > (1 << maxLogVertexCountSinceRebuild)) {  //girl what does this do. shift is higher than greater than, so 
            //when there is more than maxlodvertexcountsincerebuild -> log_2(n), so 1 << log_2(n) = 1*n
    ...
    private void remove(ConnVertex vertex) {
        ...
        if ((4 * vertexInfo.size() <= maxVertexInfoSize) && (maxVertexInfoSize > 12)) { //if it has been significantly decreased
        ...
        if ((vertexInfo.size() << REBUILD_CHANGE) <= (1 << maxLogVertexCountSinceRebuild)) { //no.current vertices * diff in curr and allocated <= current
            rebuild(); //to reallocate space
        }
    }

    /**
     * Collapses an adjacency list (either graphListHead or forestListHead) for an EulerTourVertex into the adjacency
     * list for an EulerTourVertex that represents the same underlying ConnVertex, but at a higher level. 
     * This has the effect of prepending the list for the lower level to the beginning of the list for the higher level, and
    ...
    private ConnEdge collapseEdgeList(ConnEdge head, ConnEdge lowerHead, EulerTourVertex vertex, EulerTourVertex lowerVertex) {
        ...
        ConnEdge lowerEdge = lowerHead;  //first node in teh lsit fofr lower level vertex
        while (lowerEdge != null) { //keep looping until we hit the last possible adjacent edge
            prevLowerEdge = lowerEdge;  //prev lower edge is set to lower edge. if the lower edge u = lower vertex, then we go to the next edge
            if (lowerEdge.vertex1 == lowerVertex) {
                lowerEdge.vertex1 = vertex;
                lowerEdge = lowerEdge.next1; //next adjacent edge to u
            } else {                    //else if lower edge vertex 2 = vertex, then we go to the next edge again
                lowerEdge.vertex2 = vertex;
                lowerEdge = lowerEdge.next2; //next adjacent edge to v -> vertex 2
            }
        }
        //set the head to whichever end point is vertex
        ...
        if (head != null) { //while there is an endpoint that coincides with vertex
            if (head.vertex1 == vertex) { 
                head.prev1 = prevLowerEdge; //set head.prev to prevLowerEdge, so prevLowerEdge.next = head and head.prev = prevLowerEdge
            } else {
                head.prev2 = prevLowerEdge;
            }
            //set head.prev1 or head.prev2 to the last edge in the lower level
        }
        //we basically jsut take the last possible adjacent edge and attach it to the head of the higher level
        //this means that we get [lower level edges] + [higher level edges] in the higher level 
        return lowerHead;  //after adding the lower edges to the front, this becomes the head of the combines low + high list
    }
    ...
    private void rebuild() {
        ...
        while (2 * vertexInfo.size() <= 1 << maxLogVertexCountSinceRebuild) {
            maxLogVertexCountSinceRebuild--; //if the number of vertices is less than 2^maxLogVertexCountSinceRebuild, then we can decrease the max log vertex count
            deleteCount++; //that means that we can decrease  the number of levels by 1
        }
        if (deleteCount == 0) {
            return;
        }

        for (VertexInfo info : vertexInfo.values()) { //going through all the vvertices
            EulerTourVertex vertex = info.vertex;
            EulerTourVertex lowerVertex = vertex; //representation of this edge at the lower level
            for (int i = 0; i < deleteCount; i++) { //for every level that we can delete, we collapse the occurences of this vertex at these levels into the levels below it
                lowerVertex = lowerVertex.lowerVertex;
                if (lowerVertex == null) {
                    break;
                }
                // Collapse the lower-level edges into the higher-level vertex 
                vertex.graphListHead = collapseEdgeList(vertex.graphListHead, lowerVertex.graphListHead, vertex, lowerVertex);
                if (lowerVertex.forestListHead != null) { //if thre are tree edges at the lower level
                    // Change the eulerTourEdge links
                    ConnEdge lowerEdge = lowerVertex.forestListHead; //head of the list of tree edges that contains lowervertex
                    while (lowerEdge != null) { //while there are more edges in this tree at the lower level
                        if (lowerEdge.vertex1 == lowerVertex) {
                            // We'll address this edge when we visit lowerEdge.vertex2
                            lowerEdge = lowerEdge.next1;
                        } else { //get this edge at its highest possible level and set lower edge to that
                            EulerTourEdge edge = lowerEdge.eulerTourEdge.higherEdge; 
                            //here 0 will have least number of edges, and log2n will have all edges so higher we go, more edges
                            //so we are going through the next i levels and finding the iteration of theis edge at level i

                            for (int j = 0; j < i; j++) {
                                edge = edge.higherEdge;
                            }
                            lowerEdge.eulerTourEdge = edge; //changes the edge representation to the edge at level i+1 above what it was
                            lowerEdge = lowerEdge.next2; //set lower edge to the next adjacent tree edge
                        }
                    }
                    //update the tree edges by including tree edges from the lower level that we have just rendered obsolete
                    //lower levels are smaller (?)
                    vertex.forestListHead = collapseEdgeList(vertex.forestListHead, lowerVertex.forestListHead, vertex, lowerVertex);
                }
            }
            //while the lowerVertex is not null, we bring it just one level lower
            if (lowerVertex != null) {
                lowerVertex = lowerVertex.lowerVertex;
            }
            //either vertex.lowerVertex is null, or it is the representation of this edge at the next lower level at level i - 1(?)
    ...
    public boolean addEdge(ConnVertex connVertex1, ConnVertex connVertex2) {
        ...
        if (info1.edges.containsKey(connVertex2)) {  ///edge alr exists
            return false;
        }
    ...
    public boolean is_tree_edge(ConnVertex connVertex1, ConnVertex connVertex2) {
        ...
        ConnEdge edge = info1.edges.get(connVertex2);
        if (edge == null) {
            return false;
        }
        return edge.eulerTourEdge != null;
    }
